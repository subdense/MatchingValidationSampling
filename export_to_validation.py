...
parser.add_argument('-select', type=argparse.FileType('r'), required=True, help='selection data (points used to create tasks)')
parser.add_argument('-select_layer', type=str, required=True, help='name of the layer to extract from the selection data')
...
parser.add_argument('-radius', type=float, required=True, help='radius to select features around the selection layer')
...
idb1 = geopandas.read_file(args.layer1.name, engine="pyogrio", columns=[], fid_as_index=True)
crs = idb1.crs
...
select = geopandas.read_file(args.select.name, layer=select_layer, engine="pyogrio", columns=[], fid_as_index=True).to_crs(crs)
radius = args.radius
...
def connected_components(idb1, idb2):
    ...
    join["index_right"] = join["fid_right"].map('R_{}'.format)
